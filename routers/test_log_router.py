...
@router.get('/test-log/{_id}', response_model=TestLogSelectSchema)
def get_by_id(_id: int):
    test_log = crud.get_by_id(_id=_id)
    return R.ok(data=test_log)


@router.post('/test-log', response_model=TestLogSelectSchema)
def save(item: TestLogCreateSchema):
    test_log = crud.save(schema_in=item)
    return R.ok(data=test_log)


@router.put('/test-log/{_id}', response_model=TestLogSelectSchema)
def update_by_id(_id: int, item: TestLogUpdateSchema):
    test_log = crud.update_by_id(_id=_id, schema_in=item)
    return R.ok(data=test_log)


@router.delete('/test-log/{_id}', response_model=TestLogSelectSchema)
def remove(_id):
